...
## Notes for Development (What this means)

- **CORS is open to all origins (development only):** Any frontend URL can call your backend API right now. This is convenient for local testing, but not safe for production.
- **SQLite is the default database:** All data is saved in `backend/db.sqlite3` (generated locally after running migrations). This is simple for local development, but production usually uses PostgreSQL or MySQL.
- **JWT access token lasts 60 minutes:** After login, the access token works for about 1 hour before it needs to be refreshed/reissued.

### Before deploying to production

- Move secrets (like Django `SECRET_KEY`) to environment variables.
- Set `DEBUG = False`.
- Configure `ALLOWED_HOSTS` with your real domain(s).
- Restrict CORS to trusted frontend domains only.
- Use a production-grade database and proper backup strategy.
